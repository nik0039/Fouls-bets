...
def get_all_team_matches(team, stat_teams, stats, mode, similiar_teams):
    if mode == 1:
        team_stata_home = stat_teams.loc[
            (stat_teams['Team1'] == team), list(filter(lambda st: st.find('Team1') != -1, stats))]
        ...
        team_stata_away = stat_teams.loc[
            (stat_teams['Team2'] == team), list(filter(lambda st: st.find('Team2') != -1, stats))]
        ...
        team_stata_home = stat_teams.loc[
            (stat_teams['Team1'] == team) & (stat_teams['Team2'].isin(similiar_teams)), list(
        ...
        t1 = team_stata_home.rename(columns=dict(zip(filtered, map(lambda st: st.replace('1', ''), filtered))))
        team_stata_away = stat_teams.loc[
            (stat_teams['Team2'] == team) & (stat_teams['Team1'].isin(similiar_teams)), list(
...
def clasterization(settings, cup):
    ...
    teams = np.sort(teams)
    for team in teams:
        t1, t2 = get_all_team_matches(team, stat_teams, stats, 1, None)
...
def main():
    ...
    for mode in [1, 2]:
        ...
        if mode == 1:
            t1, t2 = get_all_team_matches(Team1, stat_teams, stats, mode, None)
        elif mode == 2:
            similiar_teams1 = list(
                types_of_teams.loc[
                    (types_of_teams['Type'] == int(types_of_teams.loc[(types_of_teams['Team'] == Team2)]['Type']))][
                    'Team'])
            t1, t2 = get_all_team_matches(Team1, stat_teams, stats, mode, similiar_teams1)
        ...
        if mode == 1:
            t1, t2 = get_all_team_matches(Team2, stat_teams, stats, mode, None)
        elif mode == 2:
            similiar_teams2 = list(
                types_of_teams.loc[
                    (types_of_teams['Type'] == int(types_of_teams.loc[(types_of_teams['Team'] == Team1)]['Type']))][
                    'Team'])
            t1, t2 = get_all_team_matches(Team2, stat_teams, stats, mode, similiar_teams2)
